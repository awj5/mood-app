...
export default function Report(props: ReportProps) {
  ...
  const deleteInsightsData = async (ids: number[], hash: string) => {
    ...
    if (!hash) {
      ...
      try {
        await axios.post(
          !isSimulator ? "https://www.mood.ai/api/insights/delete" : "http://localhost:3000/api/insights/delete",
          {
            uuid: uuid,
            hash: hash,
  ...
  const send = async () => {
    ...
    // Send email to team
    try {
      await axios.post(!isSimulator ? "https://www.mood.ai/api/report" : "http://localhost:3000/api/report", {
