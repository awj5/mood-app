  ...
  const getStoredInsights = async (hash: string, uuid: string) => {
    try {
      const response = await axios.post(
        !isSimulator ? "https://www.mood.ai/api/insights" : "http://localhost:3000/api/insights",
        {
          uuid: uuid,
          hash: hash,
    ...
    if (uuid) {
      ...
      if (savedResponse && latestQueryRef.current === currentQuery) {
        ...
        if (aiResponse && latestQueryRef.current === currentQuery) {
          ...
          try {
            await axios.post(
              !isSimulator ? "https://www.mood.ai/api/insights/save" : "http://localhost:3000/api/insights/save",
              {
                uuid: uuid,
                hash: hash,
                summary: aiResponse,
